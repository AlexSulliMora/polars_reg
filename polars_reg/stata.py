...
def _stata_varname(name: str, indicators: set[str] | None = None) -> str:
    """Convert variable/interaction to Stata syntax.

    - x1:x2 -> c.x1#c.x2
    - indicator vars get i. prefix instead of c.
    """
    ind = indicators or set()
    if ":" in name:
        parts = name.split(":")
        return "#".join(f"i.{p}" if p in ind else f"c.{p}" for p in parts)
    if name in ind:
        return f"i.{name}"
    return name


def _stata_varlist(names: list[str], indicators: set[str] | None = None) -> list[str]:
    return [_stata_varname(n, indicators) for n in names]
...
def _to_reg(spec: Any, vce_opt: str) -> str:
    parts = ["reg", spec.depvar] + _stata_varlist(spec.exog, spec.indicators)
    opts = []
...
def _to_reghdfe(spec: Any, vcov: str, cluster: list[str] | None) -> str:
    parts = ["reghdfe", spec.depvar] + _stata_varlist(spec.exog, spec.indicators)
...
def _to_ivregress(spec: Any, method: str, vce_opt: str) -> str:
    ...
    exog = " ".join(_stata_varlist(spec.exog, spec.indicators)) if spec.exog else ""
...
def _to_panel_fd(
    ...
    d_exog = [f"D.{_stata_varname(v, spec.indicators)}" for v in spec.exog]
